...
async function checkRateLimit() {
  ...
  if (data.rate.remaining <= RATE_LIMIT_THRESHOLD) {
    console.log(
      `Approaching rate limit. Pausing for ${RATE_LIMIT_DELAY / 1000} seconds`
    );
...
async function getCommitters(
  ...
  const commits = await fetchAllItems(octokit.repos.listCommits, {
    owner,
    repo,
    since,
  });
  return new Set(
    commits
      ...
      .filter((login): login is string => !!login && !shouldExcludeLogin(login))
  );
}
...
export async function getContributorScores({
  ...
  const committers = await getCommitters(owner, repo, since);

  await processIssuesAndPullRequests(
    contributors,
    maintainerLogins,
    owner,
    repo,
  ...
  const sortedContributors = Object.entries(contributors)
    .filter(([login]) => committers.has(login))
...
async function processIssuesAndPullRequests(
  ...
  const issues = await fetchAllItems(octokit.issues.listForRepo, {
    owner,
    repo,
    state: "all",
    since,
  ...
  for (const item of issues) {
    if (shouldExcludeLogin(item.user.login) || !committers.has(item.user.login))
      continue;

    const [reactions, comments] = await Promise.all([
      fetchAllItems(octokit.reactions.listForIssue, {
        owner,
        repo,
        issue_number: item.number,
      }),
      fetchAllItems(octokit.issues.listComments, {
        owner,
        repo,
        issue_number: item.number,
      }),
    ...
    if (!hasThumbsDown) {
      addScore(
        ...
        item.user.login,
        item.pull_request
          ? SCORE_WEIGHTS.PR_CREATED
          : SCORE_WEIGHTS.ISSUE_CREATED
      );
    }
    ...
    if (item.pull_request) {
      const reviews = await fetchAllItems(octokit.pulls.listReviews, {
        owner,
        repo,
        pull_number: item.number,
      });
      processReviews(contributors, reviews, committers);
    }
  }
}
...
async function processComments(
  ...
  for (const comment of comments) {
    if (
      shouldExcludeLogin(comment.user.login) ||
      !committers.has(comment.user.login)
    )
      continue;

    const reactions = await fetchAllItems(
    ...
    if (!hasThumbsDown) {
      addScore(
        contributors,
        comment.user.login,
        isPullRequest ? SCORE_WEIGHTS.PR_COMMENT : SCORE_WEIGHTS.ISSUE_COMMENT
      );
    }
...
function processReactions(
  ...
  reactions.forEach((reaction) => {
    if (
      !shouldExcludeLogin(reaction.user.login) &&
      committers.has(reaction.user.login) &&
      (reaction.content === "+1" ||
      ...
      addScore(
        contributors,
        itemCreator,
        isPullRequest ? SCORE_WEIGHTS.PR_REACTION : SCORE_WEIGHTS.ISSUE_REACTION
      );
    }
  });
}
...
function processReviews(
  ...
  reviews.forEach((review) => {
    if (
      !shouldExcludeLogin(review.user.login) &&
      committers.has(review.user.login)
    ) {
...
export function getContributorPercentages(
  ...
  if (percentages.length > 0) {
    ...
    firstContributor.percentage =
      Math.round(firstContributor.percentage * 100) / 100;
  }
